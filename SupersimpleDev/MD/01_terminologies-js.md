This File - **01_terminologies-js.md** — Glossary of JavaScript terms, short one-line definitions
...
### Operator Precedence
`Operator Precedence` is the set of rules JavaScript uses to decide which operator is evaluated first. (is the recipe)
...
| ----------- | ----------- | ------------------------------------ |
| Variable    | `age`       | The variable that stores the value  |
| Value       | `25`        | The specific value stored           |
| Data        | `25`        | The information being stored        |
| Data Type   | `Number`    | The type of the value               |
...
### Character
`Character` is a single letter, number, symbol, or space inside a string. JavaScript does not have a separate Character data type; characters are stored as Strings (`"A"`, `"hello"`, `""`, `"@"` — for example).
...
### Object
`Object` is a data type and a collection of related data stored as key-value pairs, like a container. It lets us group and use multiple values together.

### Object Structure — Simple Relationship

`Object (person)` → contains → `Properties (name, age)` → which have → `Property Values ("John", 25)` → which have → `Data Types (String, Number)`
...
### Class (JavaScript)
`Class` is a blueprint or template used to create objects with shared properties and methods.

## 10. JSON, Storage & Console
